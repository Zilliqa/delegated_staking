...
contract DepositTest is Test {
    ...
    function test_DepositOnly() public {
        ...
        console.log("\nowner1 stake: ", deposit_contract.getStake(bls_pub_key_1));

        console.log("\nowner1 Unstake");
        vm.startPrank(owner1);
        gasBefore = gasleft();
        deposit_contract.unstake(bls_pub_key_1, 999 ether);
        console.log("Gas used", gasBefore - gasleft());
        vm.stopPrank();
        ...
        vm.startPrank(owner1);
        gasBefore = gasleft();
        deposit_contract.withdraw(bls_pub_key_1);
        console.log("Gas used", gasBefore - gasleft());
        vm.stopPrank();

        console.log("\n\nRolling ahead 2 epochs");
        vm.roll(block.number + deposit_contract.withdrawalPeriod() * 2);
